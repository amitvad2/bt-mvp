 ...
 * against the term class's ageMin/ageMax using the programme's termStartDate
 * as the reference date for age calculation.
 */
...
export default function ParentChildStep() {
  const { state, setParentDetails, setChildDetails, goToStep } = useGuestTermBooking();
  ...
  useEffect(() => {
    ...
    // Use termStartDate as reference for age validation
    const isValid = validateChildAge(childDob, termClass.termStartDate, termClass.ageMin, termClass.ageMax);
    if (!isValid) {
      setAgeError(
        `Your child must be between ${termClass.ageMin} and ${termClass.ageMax} years old at the start of the programme (${termClass.termStartDate}). Please check the date of birth.`
      );
    } else {
      setAgeError(null);
    }
  ...
  const onSubmit = (data: ParentChildFormData) => {
    ...
    if (termClass && !validateChildAge(data.childDateOfBirth, termClass.termStartDate, termClass.ageMin, termClass.ageMax)) {
      setAgeError(
        `Your child must be between ${termClass.ageMin} and ${termClass.ageMax} years old at the start of the programme (${termClass.termStartDate}). Please check the date of birth.`
      );
      return;
  ...
  return (
    <form className={styles.form} onSubmit={handleSubmit(onSubmit)} noValidate>
      ...
      <div className={styles.fieldGroup}>
        ...
        <p className={styles.sectionSubtitle}>
          Details for the child attending the programme. Age must be between{' '}
          {termClass?.ageMin}–{termClass?.ageMax} years at the start of the programme.
        </p>
