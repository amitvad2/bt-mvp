 ...
 *
 * Mandatory consents (all must be accepted to continue):
 ...
 *
 * Optional consents (unticked by default, never pre-ticked):
 *  - photographyPromotionalUse
 *  - emailMarketing
 *  - whatsappMarketing
 *
 * Validates: GUEST-FR-006 (6.1–6.5)
 */
...
// ============================================================
// Default consent state — mandatory false, optional false (never pre-ticked)
// ============================================================
...
export default function ConsentStep() {
  const { state, setConsents, goToStep } = useGuestBooking();

  // Initialise from context if returning to this step, otherwise defaults
  const [consents, setLocalConsents] = useState<GuestConsentRecord>(
    state.consents ?? DEFAULT_CONSENTS
  );
  const [showError, setShowError] = useState(false);

  // Check whether all mandatory consents are accepted
  ...
  const handleBack = useCallback(() => {
    // Persist current selections even when going back
    setConsents(consents);
    goToStep(3);
  ...
  return (
    <div className={styles.form}>
      ...
      {/* Error banner when mandatory consents incomplete */}
