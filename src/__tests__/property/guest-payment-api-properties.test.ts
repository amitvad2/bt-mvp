...
/** Build a future date string (tomorrow) */
    ...
    await fc.assert(
      fc.asyncProperty(
        ...
        async (firestorePrice, clientAttemptedPrice) => {
          ...
          expect(createCall.amount).not.toBe(
            firestorePrice !== clientAttemptedPrice ? clientAttemptedPrice : undefined
          );
        }
      ),
      { numRuns: 100 }
...
describe('Property 13: Zod Schema Validation Consistency', () => {
  /** Arbitrary for valid name strings (alphabetic only, guaranteed non-empty) */
  const validNameArb = fc.stringMatching(/^[A-Za-z][A-Za-z '-]{0,49}$/);

  /** Arbitrary for valid email addresses that pass Zod's email validator */
  ...
  /** Arbitrary for valid UK phone numbers (10-20 digits) */
  ...
  it('schema rejects invalid inputs consistently', () => {
    ...
    const invalidBodyArb = fc.oneof(
      ...
      // Invalid email
      fc.constant({ ...buildValidBody(), parentDetails: { ...buildValidBody().parentDetails as object, email: 'not-an-email' } }),
...
describe('Property 14: Draft Failure Triggers PaymentIntent Cancellation', () => {
  it('if draft write fails, PaymentIntent is cancelled', async () => {
    await fc.assert(
      fc.asyncProperty(
        fc.string({ minLength: 5, maxLength: 30 }).map((s) => `pi_${s.replace(/[^a-zA-Z0-9]/g, 'x')}`),
        async (piId) => {
          vi.clearAllMocks();
